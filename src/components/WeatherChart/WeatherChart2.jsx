...
const setData = (data, useOffset = false) => {
    ...
    for (let i = 0; i < data.length; i += 1) {
        ...
        chartData.push({
            ...
            rain: (data[i].rain && data[i].rain['1h']) || (data[i].snow && data[i].snow['1h'])
        });
    }
    return chartData;
};
...
const formingData = ({ yesterdays, todays, tomorrows }) => {
    ...
    return {
        line: [
            {
                id: '어제 날씨',
                data: [...subYesterdayData1, ...subYesterdayData2, ...subTodayData]
            },
            { id: '오늘 날씨', data: [...mainTodayData, ...mainTomorrowData] }
        ],
        bar: [...subYesterdayData1, ...mainTodayData, ...mainTomorrowData]
    };
};
